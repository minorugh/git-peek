            ...
            (if (and git-peek-show-diff
                ...
                (let ((tmpfile (make-temp-file "git-peek-")))
                  (shell-command
                   (format "git -C %s show %s:%s > %s"
                           git-peek--root hash git-peek--file tmpfile))
                  (prog1
              ...
              (shell-command-to-string
               (format "git -C %s show %s:%s"
                       git-peek--root hash git-peek--file)))))
...
(defun git-peek--find-root ()
  "Return git root directory or signal error."
...
(defun git-peek--preselect (root)
  ...
  (let ((abspath
         (cond
          ...
          (t nil))))
    (when abspath
      (regexp-quote (file-relative-name abspath root)))))
